...
const SIDEBAR_WIDTH_STORAGE_KEY = "veylin_sidebar_width"
const RIGHT_SIDEBAR_WIDTH_STORAGE_KEY = "veylin_right_sidebar_width"
const SIDEBAR_COOKIE_MAX_AGE = 60 * 60 * 24 * 7
const SIDEBAR_WIDTH_DEFAULT = 256
const RIGHT_SIDEBAR_WIDTH_DEFAULT = 400
...
function readStoredSidebarWidth(
  ...
  try {
    ...
    return clampSidebarWidth(value, min, getMax())
  } catch {
    return fallback
  }
}

function useSidebarWidthState(
  storageKey: string,
  fallback: number,
  min: number,
  getMax: () => number,
  ...
  const [width, setWidthState] = React.useState(() =>
    readStoredSidebarWidth(storageKey, fallback, min, getMax),
  )

  const setWidth = React.useCallback(
    (next: number) => {
      const clamped = clampSidebarWidth(next, min, getMax())
    ...
    [storageKey, min, getMax],
  )

  return { width, setWidth }
}

function sidebarWidthClass(widthVar: SidebarWidthVar) {
  return widthVar === "--right-sidebar-width"
    ? "w-(--right-sidebar-width)"
    : "w-(--sidebar-width)"
}

function sidebarContainerSideStyle(
  side: "left" | "right",
  widthVar: SidebarWidthVar,
  open: boolean,
  ...
  return side === "left"
    ? { left: `calc(-1 * var(${widthVar}))` }
    : { right: `calc(-1 * var(${widthVar}))` }
}
...
function SidebarProvider({
  ...
  const { width, setWidth } = useSidebarWidthState(
...
const RightSidebarContext = React.createContext<SidebarContextProps | null>(null)
...
function RightSidebarProvider({
  ...
  return (
    <RightSidebarContext.Provider value={contextValue}>
      <div
        style={{ "--right-sidebar-width": `${width}px` } as React.CSSProperties}
        className="contents"
      >
        {children}
      </div>
...
type SidebarPanelProps = React.ComponentProps<"div"> & {
  useSidebarHook: () => SidebarContextProps
  side?: "left" | "right"
...
function SidebarResizeHandle({
  ...
  const { open, width, setWidth } = useSidebarHook()
  ...
  const onPointerDown = (event: React.PointerEvent<HTMLButtonElement>) => {
    ...
    document.body.classList.add("sidebar-column-resizing")
...
function SidebarPanel({
  ...
  const { isMobile, state, open, openMobile, setOpenMobile, width, widthVar } =
    useSidebarHook()
  const widthClass = sidebarWidthClass(widthVar)
  const panelStyle = { [widthVar]: `${width}px` } as React.CSSProperties
  const gapStyle =
  ...
  return (
    ...
    >
      ...
      <div
        ...
        {...props}
      >
        <SidebarResizeHandle side={side} useSidebarHook={useSidebarHook} />
        <div
          data-sidebar="sidebar"
          ...
          className={cn(
            ...
            "group-data-[side=right]:bg-background group-data-[side=right]:shadow-[inset_1px_0_0_0_var(--border)]",
          )}
        >
          {children}
        </div>
...
function Sidebar(props: Omit<SidebarPanelProps, "useSidebarHook">) {
  return <SidebarPanel useSidebarHook={useSidebar} {...props} />
}

function RightSidebar(props: Omit<SidebarPanelProps, "useSidebarHook" | "side">) {
  return <SidebarPanel useSidebarHook={useRightSidebar} side="right" {...props} />
}
...
function SidebarTrigger(
  props: Omit<
    React.ComponentProps<typeof SidebarTriggerBase>,
    "useSidebarHook" | "icon"
  >
) {
  return (
    <SidebarTriggerBase useSidebarHook={useSidebar} icon={PanelLeftIcon} {...props} />
  )
}
...
function RightSidebarTrigger(
  ...
  return (
    <SidebarTriggerBase
      useSidebarHook={useRightSidebar}
      icon={PanelRightIcon}
      {...props}
    />
  )
...
function SidebarRail(
  props: Omit<React.ComponentProps<typeof SidebarRailBase>, "useSidebarHook">
) {
  return <SidebarRailBase useSidebarHook={useSidebar} {...props} />
}

function RightSidebarRail(
  props: Omit<React.ComponentProps<typeof SidebarRailBase>, "useSidebarHook">
) {
  return <SidebarRailBase useSidebarHook={useRightSidebar} {...props} />
}
