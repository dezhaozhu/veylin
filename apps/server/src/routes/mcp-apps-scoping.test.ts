...
describe('resolveScopedServerNames', () => {
  ...
  it('no threadId means no filtering (today\'s tenant-wide behavior, unchanged)', async () => {
    const allow = await resolveScopedServerNames(DEV_TENANT_ID, undefined);
    assert.equal(allow, undefined);
  });

  it('with two grouped servers and a pinned thread, only the pinned member (+ ungrouped) is scoped in', async () => {
    ...
    const allow = await resolveScopedServerNames(DEV_TENANT_ID, threadId);
    assert.ok(allow);
  ...
  it('an unpinned thread auto-pins deterministically (does not include every group member)', async () => {
