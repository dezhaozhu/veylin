/**
 * MCP server `group` field — round-trips through create/update/list against the
 * real embedded SurrealDB (same pattern as persist-audit.test.ts).
 */
import assert from 'node:assert/strict';
import { after, before, describe, it } from 'node:test';
import { closeDb, connectDb } from '@veylin/db';
import {
  buildMcpServerConfigs,
  createRemoteMcpServer,
  listActiveMcpServerNames,
  listMcpServerGroups,
  listRemoteMcpServers,
  updateRemoteMcpServer,
} from './mcp-store.js';
import { COMPASS_IDENTITY_GROUP } from './compass-identity.js';
import { DEV_TENANT_ID, ensureDevTenant } from './tenant.js';

const TENANT = DEV_TENANT_ID;

describe('mcp-store group field', () => {
  before(async () => {
    await connectDb();
    await ensureDevTenant();
  });

  after(async () => {
    await closeDb();
  });

  it('creates a server with a group and round-trips it through list', async () => {
    const created = await createRemoteMcpServer(TENANT, {
      name: `grouped-${Date.now()}`,
      transport: 'http',
      url: 'https://example.com/mcp',
      headers: {},
      enabled: true,
      group: 'compass',
    });
    assert.equal(created.group, 'compass');

    const listed = await listRemoteMcpServers(TENANT);
    const found = listed.find((s) => s.id === created.id);
    assert.equal(found?.group, 'compass');
  });

  it('PUT can change the group', async () => {
    const created = await createRemoteMcpServer(TENANT, {
      name: `regroup-${Date.now()}`,
      transport: 'http',
      url: 'https://example.com/mcp',
      headers: {},
      enabled: true,
      group: 'compass',
    });

    const updated = await updateRemoteMcpServer(TENANT, created.id, { group: 'other-project' });
    assert.equal(updated?.group, 'other-project');
  });

  it('PUT can clear the group', async () => {
    const created = await createRemoteMcpServer(TENANT, {
      name: `ungroup-${Date.now()}`,
      transport: 'http',
      url: 'https://example.com/mcp',
      headers: {},
      enabled: true,
      group: 'compass',
    });

    const updated = await updateRemoteMcpServer(TENANT, created.id, { group: null });
    assert.equal(updated?.group, undefined);
  });

  it('entries without a group get undefined — no schema break for old rows', async () => {
    const created = await createRemoteMcpServer(TENANT, {
      name: `nogroup-${Date.now()}`,
      transport: 'http',
      url: 'https://example.com/mcp',
      headers: {},
      enabled: true,
    });
    assert.equal(created.group, undefined);

    const listed = await listRemoteMcpServers(TENANT);
    const found = listed.find((s) => s.id === created.id);
    assert.equal(found?.group, undefined);
  });

  it('listMcpServerGroups maps name -> group, undefined for ungrouped', async () => {
    const groupedName = `groups-helper-grouped-${Date.now()}`;
    const ungroupedName = `groups-helper-ungrouped-${Date.now()}`;
    await createRemoteMcpServer(TENANT, {
      name: groupedName,
      transport: 'http',
      url: 'https://example.com/mcp',
      headers: {},
      enabled: true,
      group: 'compass',
    });
    await createRemoteMcpServer(TENANT, {
      name: ungroupedName,
      transport: 'http',
      url: 'https://example.com/mcp',
      headers: {},
      enabled: true,
    });

    const groups = await listMcpServerGroups(TENANT);
    assert.equal(groups[groupedName], 'compass');
    assert.equal(groups[ungroupedName], undefined);
  });
});

/**
 * Phase B Task 4: compass connects ONLY through the compass client pool
 * (compass-pool.ts, per-connection `x-compass-source`), so the shared config
 * builder every generic client goes through — tenant rebuild via
 * createMcpClient (server.ts), agent-run.ts's createMcpClient('run'), the
 * mcp-apps freshClient base (routes/mcp-apps.ts) — must never emit a config
 * for a COMPASS_IDENTITY_GROUP entry (it would be a headerless, unscoped
 * compass session). Dedicated tenant so DEV_TENANT rows created by other
 * suites can't blur the assertions.
 */
describe('buildMcpServerConfigs — compass-identity group exclusion', () => {
  const POOL_TENANT = '44444444-4444-4444-4444-444444444444';

  before(async () => {
    await connectDb();
  });

  after(async () => {
    await closeDb();
  });

  it('skips the compass-group entry; other grouped and ungrouped servers keep their configs', async () => {
    await createRemoteMcpServer(POOL_TENANT, {
      name: 'compass',
      transport: 'http',
      url: 'https://compass.example.com/mcp/',
      headers: { Authorization: 'Bearer account-token' },
      enabled: true,
      group: COMPASS_IDENTITY_GROUP,
      managed: true,
    });
    await createRemoteMcpServer(POOL_TENANT, {
      name: 'other-grouped',
      transport: 'http',
      url: 'https://other.example.com/mcp',
      headers: { 'x-api-key': 'k' },
      enabled: true,
      group: 'some-other-proj',
    });
    await createRemoteMcpServer(POOL_TENANT, {
      name: 'plain',
      transport: 'http',
      url: 'https://plain.example.com/mcp',
      headers: {},
      enabled: true,
    });

    const configs = await buildMcpServerConfigs(POOL_TENANT);

    // The compass entry is ACTIVE (it must stay visible to scoping/health by
    // name) — it just never yields a generic client config.
    const active = await listActiveMcpServerNames(POOL_TENANT);
    assert.ok(active.includes('compass'));
    assert.equal('compass' in configs, false);

    // Non-compass servers are untouched by the filter — grouped or not.
    const otherGrouped = configs['other-grouped'] as { url: URL; requestInit?: unknown };
    assert.equal(otherGrouped.url.href, 'https://other.example.com/mcp');
    assert.deepEqual(otherGrouped.requestInit, { headers: { 'x-api-key': 'k' } });
    assert.ok(configs['plain']);
  });
});
