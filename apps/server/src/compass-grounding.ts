...
export const COMPASS_GROUNDING_TEXT = [
  ...
  '',
  '**7. 说到具体位置就带人去看**——回答里点名了某道作业（job_id）、某个订单或某个资源/瓶颈时，在回答末尾调一次 `navigate`（kind=job/order/resource/rule，id 用工具结果里的编码，带上 run_id；工具结果行里有 `anchor`/`_anchor` 就原样照抄它的字段——三级工序带 op、规则带作用域；说的是一张订单时 surface=both 让表格与甘特同屏）把右侧面板带到那里。**一轮最多一次**，不重复拉右栏；用户明确说不要跳转就不调。面板找不到那条时会自己如实说，你不用替它解释。',
].join('\n');
