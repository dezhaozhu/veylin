 ...
 * Suspends the native Mastra run (same mechanism as ask_user_question); the
 * client resumes the same tool call with the picked face ids.
 */
export const request3dSelection = createTool({
  ...
  execute: async (input, ctx) => {
    ...
    if (!resumeData) {
      if (ctx?.agent?.suspend) {
        return ctx.agent.suspend({ prompt: input.prompt });
      }
      return { face_ids: [], cancelled: true };
    }
    return resumeData;
  },
});
